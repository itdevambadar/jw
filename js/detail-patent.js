...
(function () {
  const tableConfigs = {
    ...
    table2: {
      columns: [
        ...
        { name: "pctnumber", type: "input", placeholder: "Pct Number" },
        {
          name: "intlfilingdate",
          type: "datepicker",
          placeholder: "International Filing Date",
        },
        {
          name: "dlfilingorder",
          type: "input",
          placeholder: "DL Filing Order",
          readonly: true,
        },
        {
          name: "dlfilingse",
          type: "input",
          placeholder: "DL Filing Substantive Examination",
          readonly: true,
        },
    ...
    table3: {
      columns: [
        // { name: 'applicant', type: 'select', options: [{ id: 1, text: 'Applicant', address: 'address 1' }, { id: 2, text: 'Samsung Ltd', address: 'address 2' }, { id: 3, text: '-', address: 'address 3' }] },
        { name: "applicant", type: "select", options: [] },
        {
          name: "selectedValue",
          type: "input",
          placeholder: "Address",
    ...
    table10: {
      columns: [
        ...
        { name: "reason", type: "input", placeholder: "Reason/Note" },
        {
          ...
          type: "select",
          options: [
            { id: 1, text: "Translator" },
            { id: 2, text: "Sahat Manihuruk" },
            { id: 3, text: "-" },
          ],
        },
      ...
      columns: [
        {
          name: "recexaminer",
          type: "input",
          placeholder: "Recommended by Examinere",
        },
        { name: "instdate", type: "input", placeholder: "Instruction Date" },
        {
          name: "makeorderdiv",
          type: "input",
          placeholder: "Make to New Order Divisional",
        },
      ],
    },
    table24: {
      ...
      columns: [
        {
          ...
          type: "select",
          options: [
            { id: 1, text: "Applicant", address: "address 1" },
            { id: 2, text: "Samsung Ltd", address: "address 2" },
            { id: 3, text: "-", address: "address 3" },
          ],
        },
        {
          name: "selectedValue",
    ...
    table30: {
      columns: [
        // { name: 'applicantchange', type: 'select', options: [{ id: 1, text: 'tes change' }, { id: 2, text: '-' }] },
        {
          name: "applicantchange",
          type: "select",
          options: [
            { id: 1, text: "Applicant", address: "address 1" },
            { id: 2, text: "Samsung Ltd", address: "address 2" },
    ...
    button.addEventListener("click", function () {
      ...
      config.columns.forEach((column) => {
        ...
        if (column.type === "input") {
          td.innerHTML = `<input type="text" class="${column.name}" name="${
            column.name
          ...
          select.innerHTML =
            ...
            column.options
              .map(
                (option) =>
                  `<option value="${option.id}" data-address="${option.address}">${option.text}</option>`
              )
              .join("");
          select.addEventListener("change", function () {
            const selectedValueInput = formRow.querySelector(".selectedValue");
            const selectedAddress =
              this.options[this.selectedIndex].getAttribute("data-address"); // Get the address of the selected option
      ...
      document.querySelector(
        `.formSubmitButton[data-table-id='${tableId}']`
    ...
    button.addEventListener("click", function (event) {
      ...
      config.columns.forEach((column) => {
        if (column.type === "checkbox") {
          values[column.name] = formRow.querySelector(
            `.${column.name}`
          ).checked;
        } else {
      ...
      if (formRow.dataset.editing === "true") {
        ...
        const rowToEdit = document.querySelector(
          `#${tableId} tbody tr[data-editing='true']`
        );
        config.columns.forEach((column, index) => {
          if (column.type === "checkbox") {
            // For checkbox, set the checked property
            rowToEdit.cells[index + 1].querySelector("input").checked =
              values[column.name];
          } else {
  ...
  function editRow(row, tableId) {
    ...
    config.columns.forEach((column) => {
      ...
      if (column.type === "input") {
            ...
            .map(
              (option) =>
                `<option value="${option.id}" ${
                  values[column.name] === option ? "selected" : ""
                }>${option}</option>`
            )
            .join("");
        select.addEventListener("change", function () {
        ...
        td.innerHTML = `<input type="checkbox" class="${column.name}" name="${
          column.name
        }" ${values[column.name] ? "checked" : ""}>${column.label}`;
      } else if (column.type === "datepicker") {
        td.innerHTML = `<input type="date" class="${column.name}" name="${
          column.name
        }" value="${values[column.name]}" required>`;
      } else if (column.type === "file") {
        td.innerHTML = `<input type="file" class="${column.name}" name="${column.name}" required>`;
      }
      formRow.appendChild(td);
  ...
  function viewRow(row, tableId) {
    ...
    config.columns.forEach((column, index) => {
      if (column.type === "checkbox") {
        values += `${column.name}: ${
          cells[index + 1].querySelector("input").checked
        }\n`;
      } else {
        values += `${column.name}: ${cells[index + 1].textContent}\n`;
      }
  ...
  function fetchAndTransformData() {
    $.ajax({
      url: "http://localhost:8000/master/applicant",
      type: "GET",
      success: function (data) {
        const options = data.map((item, index) => ({
          id: item.id,
          text: item.c_name || "-",
          address: item.c_address || "address " + (index + 1),
        }));

        tableConfigs.table3.columns[0].options = options;
        tableConfigs.table29.columns[0].options = options;
        tableConfigs.table30.columns[0].options = options;

        console.log("Updated tableConfigs:", tableConfigs);

        // Here you can perform additional actions with tableConfigs if needed
      },
  ...
  function getMasterConsultant() {
    $.ajax({
      url: "http://localhost:8000/master/consultant",
      type: "GET",
      contentType: "application/json",
      data: JSON.stringify({
        // Your request body here if needed, otherwise you can remove `data` property
      }),
      success: function (data) {
        const options = data.map((item, index) => ({
          id: item.id,
          text: item.c_nama_konsultan || "-",
        }));

        tableConfigs.table24.columns.find(
          (column) => column.name === "consultant"
        ).options = options;
...
$(document).ready(function () {
  ...
  document
    .getElementById("toggle-edit-btn")
    .addEventListener("click", function () {
      ...
      for (var i = 0; i < cells.length; i++) {
        ...
        if (content === "<p>-</p>" || cells[i].querySelector("p") !== null) {
          if (
            cells[i].id === "fildate" ||
            cells[i].id === "printdate" ||
            cells[i].id === "deadfiling" ||
            cells[i].id === "filreport"
          ) {
            cells[i].innerHTML =
              '<input type="date"  value="" id="' + cells[i].id + '">';
          } else {
            cells[i].innerHTML =
              '<input type="text" value="" id="' + cells[i].id + '">';
          }
        }
      }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (defaultTab) {
    var defaultContentId = defaultTab
