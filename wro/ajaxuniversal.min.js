...
function themePageInit(){checkCookie();$('#smallFont').click(function(){setCookie("13");setFontSize("13");});$('#mediumFont').click(function(){setFontSize("17");setCookie("17");});$('#bigFont').click(function(){setFontSize("20");setCookie("20");});$(".rowCount").each(function(){var count=$(this).text().replace("(","").replace(")","");$(this).text(count);$(this).addClass("pull-right badge");});if($(".dataList .pagelinks a").length===0){$(".dataList .pagelinks").css("visibility","hidden");}
$(".filter-cell select").on("change",function(){if($(this).val()===""){$(this).addClass("emptyValue");}else{$(this).removeClass("emptyValue");}});$(".filter-cell select").each(function(){if($(this).val()===""){$(this).addClass("emptyValue");}else{$(this).removeClass("emptyValue");}});}
...
$(menu).parent().toggleClass("toggled");}};var initMenu=function(){$("#sidebar a.dropdown").each(function(){if($(this).parent().hasClass("active")){toogleMenu(this);}});$("#sidebar").on("click","a.dropdown",function(e){toogleMenu(this);e.preventDefault();e.stopPropagation();});$("#sidebar").on("focus","a.dropdown",function(e){$(".focusVisible").removeClass("focusVisible");if(isTabPress){var el=this;if($(el).closest("#sidebar").length>0){if(!$("body").hasClass("horizontal_menu")&&$("body").hasClass("sidebar-minimized")){$("#sidebar-trigger").trigger("click");}
if($("body").hasClass("horizontal_menu")){if($(el).next().is(":hidden")){$(el).next().addClass("focusVisible");}}else{if(!$(el).parent().hasClass("toggled")){toogleMenu(el,true);}}}}});};initMenu();$("#sidebar-trigger").on("click",function(){if($("body").width()>=1280){$("body").toggleClass("sidebar-minimized");}else{if(!$("body").hasClass("sidebar-toggled")){if($(".ma-backdrop").length===0){var backdrop='<div class="ma-backdrop" />';$("body").prepend(backdrop);}
$("body").addClass("sidebar-toggled");$(this).addClass("toggled");$("#sidebar").addClass("toggled");}else{$("body").removeClass("sidebar-toggled");$(".ma-backdrop").remove();$("#sidebar, #sidebar-trigger").removeClass("toggled");}}
return false;});$("body").off("click.sidebar-toggled");$("body").on("click.sidebar-toggled",".ma-backdrop",function(e){var container=$("#sidebar");if(($(".ma-backdrop").is(":visible")&&$("body").hasClass("sidebar-toggled")&&!$("#sidebar-trigger").is(e.target)&&$("#sidebar-trigger").has(e.target).length===0&&!container.is(e.target)&&container.has(e.target).length===0)){$("body").removeClass("sidebar-toggled");$(".ma-backdrop").remove();$("#sidebar, #sidebar-trigger").removeClass("toggled");}
return false;});if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){$("html").addClass("ismobile");}
if(window.self!==window.top){$("#responsiveSwitch").hide();}});$(window).on("load",function(){setTimeout(function(){loadInbox();$('img[data-lazysrc]').each(function(){$(this).attr('src',$(this).attr('data-lazysrc'));});},0);});$("body").on("page_loaded.inbox",function(){setTimeout(function(){loadInbox();},1);});function loadInbox(){if($(".inbox-notification").length===1){loadInboxData();$(".inbox-notification .refresh").on("click",function(e){e.preventDefault();loadInboxData();return false;});}}
let inboxReloadTimeout=null;function loadInboxData(){$(".inbox-notification .loading").show();var url=$(".inbox-notification").data("url");$.getJSON(url+"&_t="+(new Date()),{},function(data){var count=0;if(data.count!==undefined){count=data.count;}
$(".inbox-notification > a > .badge").text(count);$(".inbox-notification .dropdown-menu-title .count").text(count);$(".inbox-notification > ul > li.task").remove();if(data.data){var footer=$(".inbox-notification > ul .dropdown-menu-sub-footer").parent();var link=$(".inbox-notification > ul .dropdown-menu-sub-footer").attr("href");$.each(data.data,function(i,d){var html="<li class=\"task\"><a href=\""+link+"?_mode=assignment&activityId="+d.activityId+"\">";html+="<span class=\"header\">"+d.activityName+"</span>";html+="<span class=\"message\">"+d.processName+"</span><span class=\"time\">"+d.dateCreated+"</span>";html+="</a></li>";footer.before($(html));});$(".inbox-notification").trigger("inbox_notification_updated");}
$(".inbox-notification .loading").hide();clearTimeout(inboxReloadTimeout);inboxReloadTimeout=setTimeout(function(){loadInboxData();},300000);});}}(window.jQuery);
